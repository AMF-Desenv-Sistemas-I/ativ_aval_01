...
function somar(...numeros) {
  let result = 0; // Inicia no 0
  numeros.forEach((prod) => (result = result + prod)); // passa por todos itens somando
...
function calcularMedia(...numeros) {
  let result = 0; // Inicia no 0
  numeros.forEach((prod) => (result += prod));// passa por todos itens somando
  return result / numeros.length; // faz o calculo da média e retorna
}
...
function adicionarPropriedades(objeto, ...novasPropriedades) {
  let teste = { ...objeto }; // inicia um objeto desestruturando o objeto recebido
  novasPropriedades.map((props) => {
    teste = { ...teste, ...props }; // passa por cada item do array recebido desestruturando ele e adicionando no array
  });
  return teste;
}
